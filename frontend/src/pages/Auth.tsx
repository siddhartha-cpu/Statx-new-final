...
export default function Auth() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (user) => { beginSession(); queryClient.setQueryData(["me"], user); navigate("/app"); },
    onError: (error: Error) => toast.error(error.message || "Unable to sign in"),
  });

  return (
    <main className="relative grid min-h-svh bg-[#0a0a0a] lg:grid-cols-[1fr_520px]" data-testid="auth-page">
      <button data-testid="auth-theme-toggle" aria-label={`Switch to ${resolvedTheme === "dark" ? "light" : "dark"} mode`} onClick={() => setTheme(resolvedTheme === "dark" ? "light" : "dark")} className="absolute right-5 top-5 z-20 grid size-10 place-items-center rounded-xl border border-slate-200 bg-white text-slate-600 shadow-sm hover:text-indigo-600">{resolvedTheme === "dark" ? <Sun className="size-4" /> : <Moon className="size-4" />}</button>
      <section className="relative hidden overflow-hidden border-r border-white/10 p-12 lg:flex lg:flex-col lg:justify-between">
        ...
        <div className="relative z-10 flex items-center gap-3" data-testid="auth-brand">
          <div className="grid size-9 place-items-center rounded-md bg-[#0055ff] font-mono text-sm font-bold">SN</div>
          <span className="font-mono text-sm tracking-[0.24em] text-white">STATNEX AI</span>
        </div>
        <div className="relative z-10 max-w-xl">
          ...
          <p className="mt-7 max-w-lg text-base leading-7 text-zinc-400">StatNex AI brings chat, document intelligence, learning actions, and current research into one focused workspace.</p>
        </div>
      ...
      <section className="flex items-center justify-center p-6 sm:p-12">
        <div className="w-full max-w-sm" data-testid="auth-form">
          <div className="mb-10 lg:hidden" data-testid="auth-mobile-brand"><div className="mb-5 grid size-9 place-items-center rounded-md bg-[#0055ff] font-mono text-sm font-bold">SN</div><p className="font-mono text-xs tracking-[0.24em] text-white">STATNEX AI</p></div>
          <div className="mb-9">
            ...
            <p className="mt-2 text-sm leading-6 text-zinc-500">{registering ? "Create your StatNex AI workspace. AI infrastructure stays server-side." : "Continue where your thinking left off."}</p>
          </div>
          ...
          <button data-testid="auth-mode-toggle" className="mt-9 w-full text-center text-sm text-zinc-400 hover:text-white" onClick={() => setRegistering((value) => !value)}>{registering ? "Already have a workspace? Sign in" : "New to StatNex AI? Create a workspace"}</button>
        </div>
      </section>
    </main>
  );
